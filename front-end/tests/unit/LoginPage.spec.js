...
describe ('LoginPage.vue', () => {
  ...
  beforeEach(() => {
   ...
   buttonSubmit = wrapper.find('from button [type="submit"]')
  ...
  it ('should render login form', () => {
    expect(wrapper.find('logo').attriutes().src)
    ...
    expect(wrapper.find('.link-sign-up').attriutes().href)
    .toEqual('/register')
  ...
  it('should succed when credentials are valid', async () => {
    expect.assertions(2)
    const stub = jest.fn()
  ...
  it('should fail when credentials are invalid', async () => {
    expect.assertions(3)
    // In the mock, only passwrod `JestRocks!` combined with
    // username `sunny` or `suny@taskagile.com` is valid
    wrapper.vm.form.username = 'sunny'
  ...
  it ('should have validations to prevent invalid data submit', () => {
    // Empty form
    wrapper.vm.submitForm()
    ...
    wrapper.vm.form.password = "My Password"
