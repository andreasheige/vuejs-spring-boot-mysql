...
public class HibernateUserRepository extends HibernateSupport implements UserRepository{
    ...
    public User findByUsername(String username) {
        Query<User> query = getSessions().createQuery("from User where username = :username", User.class);
        query.setParameter("username", username);
        return query.uniqueResult();
    }
    @Override
    public User findByEmailAddress(String emailAddress){
        Query<User> query = getSessions().createQuery("from User where emailAddress = :emailAddress", User.class);
